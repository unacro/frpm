...
FRP_CACHE_DIR="/usr/share/frp/cache"
...
configure_frpc() {
    ...
    if [ -n "$new_server_token" ]; then
        # sed -i "s/^(auth\.|#)?\s*token\s*=.*$/token = ${new_server_token}/" "${FRP_INSTALL_DIR}/frpc.toml" # todo 严格正则未通过
        sed -i "/\[auth\]/{n;s/.*token\s*=.*/token = \"${new_server_token}\"/}" "${FRP_INSTALL_DIR}/frpc.toml"
    fi
    echo "已更新配置"
}
...
main() {
    ...
    select choice in "${options[@]}"; do
        case $REPLY in
        1)
            configure_frpc # 默认配置 frpc
            ;;
        2)
            reload_frpc # 默认重载 frpc
            ;;
        3)
            check_frpc # 只有客户端可以使用 status 命令查看代理运行状态
            ;;
        4)
            upgrade_frp
            ;;
